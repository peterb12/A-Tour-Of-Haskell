...
and if you type `complicatedFourteen` in the REPL, it will also reply with `14`. So we gave the name `complicatedFourteen` to the expression `1 + 5 + 8`, and Haskell will remember that.  What we name expressions is entirely our choice (although they have to start with a lower-case letter.)
